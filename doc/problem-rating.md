...
聚焦测试覆盖 AC 顺序与拆分题族、滑窗/核/IRT 特征、未提交样本口径、题目位置和 `teamSizeMedian` 特征排除、预测特征不依赖队伍规模、模型集成门控、XCPC 标识稳定性、本地路径隔离、静态投影、缺失题名规范化和离线生成。Node 测试覆盖 schema（含空题名）、场次筛选、两类双向排序、自然题号、曲线难度顺序、题量比例、单调路径、URL 状态、独立数据加载和窄屏复合题目身份列。默认测试只读取构造数据，不访问网络，也不依赖已同步的本地缓存。模型精度数字来自当前离线训练特征的重新验证；静态发布不会重新训练模型。
