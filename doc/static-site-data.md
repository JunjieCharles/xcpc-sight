...
宽表、单场表和前瞻表仅在 DOM 中保留视口附近的行；筛选仍在已加载数据上执行。虚拟表使用显式 `colgroup`、固定 table layout 和固定 44px 行高。所有横向滚动表的冻结区均以不超过其滚动容器宽度的 50% 为硬条件。前瞻表总宽由实际列数组计算；学校、队名、成员和奖牌列的桌面宽度分别为 `160px`、`170px`、`190px` 和 `150px`。容器宽于 `1040px` 时冻结前三列，否则只冻结学校和队名；两列宽度分别为 `min(112px, 23cqw)`、`min(128px, 27cqw)`，总表宽随冻结列同步收缩以防止列宽被自动重新分配。系列与前瞻 JSON 当前仍一次性加载和解析；本版不引入服务端搜索或分块。
