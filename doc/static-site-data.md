...
1. **系列宽表**：按比赛顺序扫描；未参加时从系列初始 Rating `1400` 开始沿用，实际参赛时更新为 `after`。前四列依次为排名、选手/学校、最终 Rating、参赛次数并冻结，其后为比赛列；实际参赛单元格显示 `Rating (delta)`，未参赛单元格只显示沿用 Rating。
2. **单场参赛者表**：筛选 `contestIndex`，按比赛排名、选手/学校、`before`、`after`、`delta` 的顺序展示。
3. **选手完整曲线**：覆盖系列所有比赛；从初始 Rating 开始，参赛点更新为 `after`，未参赛点水平延续，只有实际参赛点显示 marker。
