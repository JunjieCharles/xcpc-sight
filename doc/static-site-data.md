...
宽表、单场表和前瞻表仅在 DOM 中保留视口附近的行；筛选仍在已加载数据上执行。虚拟表使用显式 `colgroup`、固定 table layout 和固定 44px 行高。前瞻表总宽由实际列数组计算；学校、队名、成员和奖牌列的桌面宽度分别为 `160px`、`170px`、`190px` 和 `150px`，桌面冻结前三列。视口不超过 `700px` 时学校和队名继续冻结，成员列取消冻结；两列宽度分别为 `min(112px, 23cqw)`、`min(128px, 27cqw)`，合计不超过表格滚动容器的 50%，总表宽随冻结列同步收缩以防止列宽被自动重新分配。系列与前瞻 JSON 当前仍一次性加载和解析；本版不引入服务端搜索或分块。
