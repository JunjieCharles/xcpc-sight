...
宽表、单场表和前瞻表仅在 DOM 中保留视口附近的行；筛选仍在已加载数据上执行。虚拟表使用显式 `colgroup`、固定 table layout 和固定 44px 行高。含综合战力列的前瞻表总宽 `1214px`，学校、队名、成员和奖牌列分别为 `160px`、`170px`、`190px` 和 `150px`；桌面冻结前三列。视口不超过 `700px` 时学校和队名继续冻结，成员列取消冻结；两列宽度分别为 `min(112px, 23vw)`、`min(128px, 27vw)`，合计不超过视口的 50%，表格总宽为其与其余列 `884px` 之和，防止列宽被自动重新分配。系列与前瞻 JSON 当前仍一次性加载和解析；本版不引入服务端搜索或分块。
