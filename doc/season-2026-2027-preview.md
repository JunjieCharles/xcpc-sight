...
队伍表每个评分单元格显示该项已匹配队员中的最高值，并在同一行以 `#x` 显示该值在全部报名队伍中的名次；各数据列为小号等宽数字排名保留相同的紧凑窄槽，主值右对齐，排名在窄槽内左对齐并紧随主值。同值使用竞赛排名并列，后续名次留空位，没有任何匹配时显示 `—` 且不显示名次。Rating 不使用千分位，XCPC Rating 始终保留两位小数。XCPC Rating 使用其深牛津蓝视觉，XCPC Elo 使用其等级色与橙色定义，其中 `>=3000` 的首位数字为黑色、其余数字为红色；上赛季 Rating 沿用本站选手 Rating 的 Codeforces 风格等级色；CPC Finder 保持普通文本色。鼠标正常悬浮或键盘聚焦单元格会展示所有队员在该来源的值，界面不显示额外提示图标，未匹配成员仍显示 `—`。金、银、铜牌数来自 CPC Finder，按队员求和为队伍总数；奖牌显示时仅以空白分隔三个图标及数值，排序和名次均依次比较金牌、银牌、铜牌。
