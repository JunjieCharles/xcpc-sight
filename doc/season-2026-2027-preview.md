...
当前快照约 1.7 MB，一次性加载后以虚拟滚动表显示。页面不展示内部快照日期、匹配政策或静态快照说明，名单来源与四项数据来源分成两行。前瞻始终作为独立 tab 显示，即使当前系列没有其他页面；标题区保留比赛选择下拉框，本期仅含网络赛第一场，后续增加前瞻场次时扩展选项。所有前瞻列表头文字居中。四项 Rating 和奖牌名次以全部队伍为基数预先计算，搜索和学校筛选不会重新排名。容器宽于 `1040px` 时冻结学校、队名和成员三列，否则只冻结学校和队名；成员列只按三名三字姓名预留紧凑宽度。窄容器下学校和队名列分别取 `min(112px, 23cqw)` 与 `min(128px, 27cqw)`，横屏和竖屏的冻结总宽均不超过表格滚动容器的 50%；表格总宽从实际列数组计算并随冻结列同步收缩，防止浏览器重新分配列宽。成员明细与综合战力悬浮窗固定到视口并根据剩余空间显示在单元格上方或下方，避免被表格滚动容器裁切。它不是比赛结果预测，不给未匹配选手填新人先验，也不承诺外部评分体系彼此可比；综合战力仅是上述五维相对支配计数的确定性排序。测试覆盖新赛季 collection、前瞻 schema、真实 2535 队快照、搜索、各列排序、各数据列全局名次、综合战力五元组、奖牌关键字顺序、缺失值顺序、URL 状态、子路径加载与响应式布局。
