...
默认写入 `static/data/problem-rating/index.json` 以及三个 series 文件。当前发布模型标识为 `gaussian-prev1-3-shallow-gbr-no-order-no-team-size`。该命令不访问网络，不修改选手 Rating JSON，也不会发布账号、队伍或逐人提交数据。后续预测的 ICPC + CCPC 队伍 Rating 使用已匹配非教练队员最终 Rating 的归一化 LSE；缺失成员不计入求和和人数，全员缺失则排除该队。已有题目预测结果按用户要求保留，尚未用新聚合重算。
...
然后打开 `http://localhost:8000/`。站点没有 npm 依赖、构建步骤或 `package.json`；可直接部署整个 `static/` 目录。2026–2027 系列已发布 ICPC 网络赛第一、第二场及 CCPC 网络预选赛的累计 8929 名选手 Rating，并提供三场赛前前瞻：ICPC 第一场 2535 支、第二场 2636 支公开报名队伍，以及 CCPC 网络预选赛的 2173 支公开榜单队伍（含 4 支打星队伍），展示学校、中文队名、非教练成员、XCPC Rating、XCPC Elo、本仓库上赛季 Rating、CPC Finder 评分及队员奖牌总数；第二场在上赛季 Rating 后增加本赛季 Rating，标题区提供比赛选择下拉框，支持通过索引扩展多场次。第一场评分列保留队内最高值；第二场起除 CPC Finder 外，队伍评分采用尺度 400 的归一化 LSE，仅聚合有评分的成员，全员缺失保留空值。CPC Finder 仍取最高值。评分不使用千分位，队伍分统一固定两位小数；个人明细中 XCPC Elo、上赛季和本赛季 Rating 的整数值按整数显示，XCPC Rating 与 CPC Finder 固定两位小数，除 CPC Finder 外沿用各来源配色；各项 Rating 与奖牌值后以 `#x` 单行显示该项在全部队伍中的并列名次，各列为名次保留相同的窄槽，筛选不重新计算。无需提示图标，正常悬浮或键盘聚焦即可查看全员明细。CPC Finder 的零分显示为 `0.00`、缺失值显示为 `—`；零分参与该列排名与数值排序，缺失值无名次且始终置后；综合战力中两者仍同档、互不超过。综合战力统计每队在各项 Rating 和奖牌上分别严格超过的队伍数，将计数降序后按元组排序；第一场五维、第二场六维，该列显示全局名次并在悬浮时展示对应五边形或六边形雷达图，页面默认按此排序。两场均在所有综合战力评分缺失且奖牌全零时显示“—”，不展示排名、雷达图或可聚焦控件；CPC Finder 零分在此判定中视为缺失，其他 Rating 零分仍有效。所有表头均可双向排序，奖牌按金、银、铜依次捆绑排序。名单来源与数据来源分开显示，前瞻始终有独立 tab；桌面冻结紧凑的学校、队名和成员列，手机冻结进一步压缩的学校和队名、取消成员冻结。悬浮窗会根据视口空间显示在格子上方或下方，不被表格底部裁切。前瞻名单是外部静态快照；ICPC 两场来自报名公示，CCPC 场来自 Pintia 公开榜单的队伍身份，也不随比赛自动更新；后续只按用户明确指定的场次或名单更新。完整口径见 [2026–2027 赛季与前瞻](doc/season-2026-2027-preview.md)。
...
“赛后复盘”已包含 ICPC 两场及 CCPC 网络预选赛；CCPC 复盘排除 4 支打星队伍，覆盖 2169 支正式队伍，其中 2109 支有提交。复盘同时展示各项指标（首场六项，后续七项）的 Spearman ρ、有效队伍数和覆盖率，点击指标卡片切换队伍表。先排除无提交队伍，再排除当前指标缺失队伍；CPC Finder 和奖牌保留零值记录。综合战力保留原前瞻顺序，排除五维计数全零队伍。两侧名次在有效队伍中重新编号并保留并列，显示箭头名次变化及 `log10(前瞻排名 / 实际排名)`；搜索和学校筛选只影响显示。网络赛第一场综合战力有效队伍为 1950 支，符合度 0.736。详见 [赛后复盘设计](doc/post-contest-review.md)。

```bash
python scripts/generate_review_data.py
```

该命令只下载文档指定版本的首场 SRK，核对 SHA-256 后生成 `static/data/reviews/2026-2027.json`，不刷新赛前评分或索引；使用 `--srk` 可完全离线生成，其他场次参数见设计文档。纯 API `core.project_review_contest(preview, contest, *, normalizer=None, overrides=None, removed_results=None)` 负责队伍关联与结果投影，不访问网络或文件。官方修订中移除的队伍须用 `removed_results`（CLI 为 `--removed-results`）显式提供“前瞻 ID → 旧结果 ID”映射，记录为已移除而非无提交；其他匹配失败仍报错。`rating.project_static_data_index(..., preview_publications=..., review_publications=...)` 负责生成包含复盘入口的索引；同系列多份前瞻会生成 `previews: [{id, path}]`，保留首项 `previewPath` 兼容入口。
...
2025–2026 赛季来自 `icpc2025` 和 `ccpc2025`；2026–2027 赛季对应 `icpc2026` 和 `ccpc2026`，当前已发布 ICPC 网络赛第一、第二场及 CCPC 网络预选赛选手 Rating，以及 ICPC 两场和 CCPC 网络预选赛的赛后复盘，并提供上述两场及 CCPC 网络预选赛的三场赛前前瞻，尚未发布本赛季题目难度；新赛季 Rating 从 1400 独立起算。两个正式赛季都按统一规则排除邀请赛、保留区域赛，并按上海本地日期排序。
...
队伍聚合公开 API：`from rating import normalized_lse_rating`，计算 `400 * log10(mean(10 ** (R / 400)))`，空输入返回 `None`；调用方过滤缺失成员。个人与报名实体赛季 Rating 的计算方式不变，详见 [Rating 规则](doc/rating-rules.md)。

2026 年 CCPC 网络预选赛前瞻已加入比赛选择，时间为北京时间 2026-09-19 13:00–18:00，覆盖 2173 支队伍、439 所学校、6443 个队员条目。本赛季 Rating 截至 ICPC 网络赛第二场，XCPC Rating 和 Elo 使用 9 月 18 日获取的最新发布。4 支打星队伍保留数据并按指标插入表格，用紫色背景和 ★ 标识；不计综合战力、各项评分、奖牌及学校排名，也不影响正式队伍的名次。名单快照及再生成命令见[前瞻设计文档](doc/season-2026-2027-preview.md#2026-ccpc-网络预选赛)。
